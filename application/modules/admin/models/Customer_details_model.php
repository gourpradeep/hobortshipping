...
class Customer_details_model extends CI_Model {
    ...
    private function _get_query() {
        ...
        $this->db->select('o.*','u.full_name','u.userID');
        $this->db->from(ORDERS .' as o');
        $this->db->join(USERS. ' as u','u.userID = o.user_id');
    ...
    function get_list() {
        ...
        if (!empty($_POST['status'])) {
            ...
            if (!empty($_POST['status'])) {
             //pr($_POST['status']);
                $this->db->where_in('service_type', $_POST['status']);
            } else {
                $this->db->or_where('service_type', $_POST['status']);
            }
            $this->db->group_end();
        }
        if (!empty($_POST['status_id'])) {
            ...
            if (!empty($_POST['status_id'])) {
             //pr($_POST['status']);
                $this->db->where_in('o.status', $_POST['status_id']);
            } else {
                $this->db->or_where('o.status', $_POST['status_id']);
            }
            $this->db->group_end();
        }
